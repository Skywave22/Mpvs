    ...
    SubtitleDelayCardContent(
      ...
      autoSyncContent = {
        AutoSyncSection(
          ...
          status = syncStatus,
          onAutoSync = {
            val subtitlePath = SubtitleAutoSync.currentExternalSubtitlePath()
            if (subtitlePath == null) {
              syncStatus = context.getString(R.string.player_sheets_sub_delay_autosync_need_external)
              return@AutoSyncSection
            }
            val videoSource = MPVLib.getPropertyString("path") ?: return@AutoSyncSection
            val timePos = MPVLib.getPropertyDouble("time-pos") ?: 0.0
            val duration = MPVLib.getPropertyDouble("duration") ?: 0.0

            // Prefer scanning from the current position; if the user is still in
            // the opening (song), start a bit further in to avoid music.
            val scanLen = 150.0
            var scanStart = timePos
            if (scanStart < 100.0) scanStart = minOf(160.0, duration * 0.15)
            if (duration > 0 && scanStart + scanLen > duration - 5) {
              scanStart = (duration - scanLen - 5).coerceAtLeast(0.0)
            }

            isSyncing = true
            syncStatus = context.getString(R.string.player_sheets_sub_delay_autosync_running)
            scope.launch {
              try {
                ...
                val dir = context.getString(
                  if (result.subsWereLateBySeconds >= 0) {
                    R.string.player_sheets_sub_delay_autosync_late
                  } else {
                    R.string.player_sheets_sub_delay_autosync_early
                  },
                )
                val conf = context.getString(
                  when (result.confidence) {
                    SubtitleAutoSync.Confidence.HIGH -> R.string.player_sheets_sub_delay_autosync_conf_high
                    SubtitleAutoSync.Confidence.MEDIUM -> R.string.player_sheets_sub_delay_autosync_conf_medium
                    SubtitleAutoSync.Confidence.LOW -> R.string.player_sheets_sub_delay_autosync_conf_low
                  },
                )
                syncStatus = context.getString(
                  R.string.player_sheets_sub_delay_autosync_success,
                  kotlin.math.abs(result.subsWereLateBySeconds),
                  dir,
                  conf,
                )
              } catch (e: Exception) {
                syncStatus = e.message ?: "Auto-sync failed"
              } finally {
                isSyncing = false
              }
            }
...
private fun AutoSyncSection(
  ...
  Column(
    ...
    Button(
      ...
      Text(stringResource(R.string.player_sheets_sub_delay_autosync))
    }
    status?.let {
      Text(
        it,
...
private fun SubtitleDelayCardContent(
  ...
  onApply: () -> Unit,
  onReset: () -> Unit,
  autoSyncContent: @Composable ColumnScope.() -> Unit = {},
) {
    DelayCardContent(
      delay = delay,
      ...
      autoSyncContent = autoSyncContent,
      extraSettings = {
